...
void Com_Meminfo_f( void ) {
	...
	int			botlibBytes, rendererBytes, fxBytes;
	int			unused;
	...
	Com_Printf( "        %8i bytes in dynamic FX\n", fxBytes );
	Com_Printf( "        %8i bytes in dynamic other\n", zoneBytes - ( botlibBytes + rendererBytes - fxBytes ) );
...
void Com_InitZoneMemory( void ) {
	...
	if ( cv->integer < 64 ) {
		s_zoneTotal = 1024 * 1024 * 64;
	} else {
		s_zoneTotal = cv->integer * 1024 * 1024;
	}

	s_zoneTotal = 64 * 1024 * 1024;
	// bk001205 - was malloc
...
void *Hunk_Alloc( int size, ha_pref preference ) {
	...
	if ( hunk_low.temp + hunk_high.temp + size > s_hunkTotal ) {
		...
		cvar_t *com_hunkMegs = Cvar_Get("com_hunkMegs", DEF_COMHUNKMEGS, CVAR_LATCH | CVAR_ARCHIVE);
#ifdef HUNK_DEBUG
		Hunk_Log();
		Hunk_SmallLog();
#endif
...
void Com_Init( char *commandLine ) {
	...
	Com_Printf( "%s %s %s\n", Q3_VERSION, ARCH_STRING, __DATE__ );
...
static void FindMatches( const char *s ) {
	...
	/* Finalize the shortest match string */
	shortestMatch[i] = 0;
}
...
void Field_CompleteCommand( field_t *field ) {
	...
	Alias_CommandCompletion( FindMatches );
	...
	Alias_CommandCompletion( PrintMatches );
}
