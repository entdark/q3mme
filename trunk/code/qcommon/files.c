...
#define	MAX_SEARCH_PATHS	8096
#define MAX_FILEHASH_SIZE	2048
...
int FS_Read( void *buffer, int len, fileHandle_t f ) {
	...
	if (fsh[f].zipFile == qfalse) {
		...
		int retSize = unzReadCurrentFile(fsh[f].handleFiles.file.z, buffer, len);
		if ( retSize < 0) { 
			Com_Printf("Unzip error %d\n", retSize );
		}
		return retSize;
	}
}
...
int FS_ReadFile( const char *qpath, void **buffer ) {
	...
	buf = Hunk_AllocateTempMemory(len+1);
//	buf = (byte *)malloc( len + 32768 ); // hilarious hack, will crash
	*buffer = buf;
...
void FS_FreeFile( void *buffer ) {
	...
	Hunk_FreeTempMemory( buffer ); /// NTxC: TODO: remove this shit
//	free( buffer );
...
static void FS_QuickSwap( char **list, int a, int b ) {
	char *old = list[a];
	list[a] = list[b];
	list[b] = old;
}

static int FS_QuickPartition( char **list, int left, int right ) {
	int first = left;
	int pivot = right;
	while( left <= right ) {
		while( strcmp( list[left], list[pivot])< 0)
			left++;
		while( (right>=first) && ( strcmp( list[right], list[pivot]) >= 0))
			right--;
		if ( left < right ) {
			FS_QuickSwap( list, left, right);
			left++;
		}
	}
	if(left!=pivot)
		FS_QuickSwap(list, left, pivot);
	return left;
}

static void FS_QuickSort( char **list, int left, int right ) {
	int pivot;

	if ( left >= right)
		return;

	pivot = FS_QuickPartition( list, left, right );
	FS_QuickSort( list, left, pivot - 1 );
	FS_QuickSort( list, pivot + 1, right );
}
...
char **FS_ListFilteredFiles( const char *path, const char *extension, char const *filter, int *numfiles ) {
	int				nfiles;
	char			**listCopy;
		...
		if (search->pack) {
			...
			for (i = 0; i < pak->numfiles; i++) {
				...
				if (filter) {
					...
					if (!Com_Filter( extension, name + length - extensionLength, qfalse ))
						continue;
...
//					}
	...
	*numfiles = nfiles;
	...
	listCopy[i] = NULL;
	/* Sort the file list */
	FS_QuickSort( listCopy, 0, nfiles-1 );
	return listCopy;
}
...
static void FS_AddGameDirectory( const char *path, const char *dir ) {
	...
	for ( i = 0 ; i < numfiles ; i++ ) {
		...
		Q_strncpyz(pak->pakPathname, path, sizeof(pak->pakPathname));
...
static void FS_Startup( const char *gameName ) {
  ...
  // NOTE: same filtering below for mods and basegame
	...
	/* Read the fs_extragames after the baseq3 */
	...
	Com_Printf ( "Current search path is not printed, use the path command if you want to see it." );
