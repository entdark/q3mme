...
typedef struct {
	...
	void	(*AddSpriteToScene)( const refSprite_t *rs );
	void	(*AddBeamToScene)( const refBeam_t *rb );
	void	(*AddRingsToScene)( const refRings_t *rr );
	void	(*AddModelToScene)( const refModel_t *rm );
	...
	void	(*DecalReset)( void );
	void	(*DecalProject)( qhandle_t shader, const vec3_t origin, const vec3_t dir, const vec4_t color, float orientation, float radius, int flags, int lifeTime );
...
typedef struct {
	...
	int		(*FS_FileIsInPAK)( const char *name, int *pCheckSum, char *pakName );
