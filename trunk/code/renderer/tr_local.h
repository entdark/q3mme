...
#define VEC_SIMD	3
typedef float vecSimd_t[VEC_SIMD];
...
CaNaBiS - q3mme
17-31 : sorted shader index
16    : is a model bit, uses model object with sublinks to the actual surface
7-15  : model surface index, and used to sort impact marks in the same order every frame
2-6   : fog index
1   :  have lightmap bit, group surfaces with lightmaps
*/

#define	QSORT_SHADER_BITS		14
#define	QSORT_FOG_BITS			5
#define	QSORT_INDEX_BITS		8

#define	MAX_SHADERS				( 1 << QSORT_SHADER_BITS )
#define QSORT_MAX_FOGS				(( 1 << QSORT_FOG_BITS ) - 1)
#define QSORT_MAX_INDEX			(( 1 << QSORT_INDEX_BITS ) - 1)
#define	MAX_REF_LIGHTS			32

#define	QSORT_SHADER_SHIFT		( 31 - QSORT_SHADER_BITS )
#define	QSORT_FOG_SHIFT			( QSORT_SHADER_SHIFT - QSORT_FOG_BITS )
#define	QSORT_INDEX_SHIFT		( QSORT_FOG_SHIFT - QSORT_INDEX_BITS )
#define QSORT_HAVELIGHT_SHIFT	( QSORT_INDEX_SHIFT - 1 )
#define QSORT_HAVEMODEL_SHIFT	( QSORT_INDEX_SHIFT - 2 )

#define QSORT_SHADER_MASK		( ( MAX_SHADERS - 1) << QSORT_SHADER_SHIFT )
#define QSORT_FOG_MASK			( QSORT_MAX_FOGS << QSORT_FOG_SHIFT )
#define QSORT_INDEX_MASK		( QSORT_MAX_INDEX << QSORT_INDEX_SHIFT )
#define QSORT_HAVELIGHT_MASK	( 1 << QSORT_HAVELIGHT_SHIFT )
#define QSORT_HAVEMODEL_MASK	( 1 << QSORT_HAVEMODEL_SHIFT )
...
typedef enum {
	...
	CGEN_FULL_IDENTITY_LIGHTING,
	CGEN_IDENTITY,			// always (1,1,1,1)
	CGEN_FULL_IDENTITY,
	CGEN_EXACT_VERTEX,		// tess.vertexColors
	CGEN_FULL_EXACT_VERTEX,		// tess.vertexColors
...
typedef struct {
	...
	union {
		...
		waveForm_t		wave;
		texModScroll_t	scroll;
		texModTransform_t transform;
		texModRotate_t	rotate;
		float			scale[2];			// s *= scale[0]
		float			rotateInteral;       // t *= scale[1]
	} data;
} texModInfo_t;
...
#define SHADER_NEED_NORMAL		0x001
#define SHADER_NEED_TEXTURE		0x002
#define SHADER_NEED_LIGHTMAP	0x004
#define SHADER_NEED_COLOR		0x008
#define SHADER_NEED_TIME		0x010
#define SHADER_NEED_ENTITY		0x020
...
typedef struct shader_s {
	...
	int 		sort;					// lower numbered shaders draw before higher numbered
...
//	int	clampTime;                                  // time this shader is clamped to
...
typedef struct sceneModel_s {
	struct sceneModel_s	*next;
	struct model_s	*model;

	int			renderfx, shaderTime;
...
typedef struct scenePortal_s {
	struct scenePortal_s	*next;

	vec3_t		origin;
	vec3_t		pvsOrigin;
	vec3_t		axis[3];
	float		rotate, rotateSpeed, rotateOffset;
} scenePortal_t;
...
typedef struct {
	...
	int				numDlights;
	dlight_t		dlights[MAX_REF_LIGHTS];
} trRefdef_t;
...
typedef struct {
	const sceneModel_t *model;
	const surfaceType_t	*surface[0];
} drawModel_t;

typedef struct drawSurf_s {
	unsigned				sort;			// bit combination for fast compares
	const void				*data;			// any of surface*_t
} drawSurf_t;
...
typedef struct srfEntity_s {
	surfaceType_t	surfaceType;
	struct srfEntity_s	*next;
} srfEntity_t;
...
typedef struct {
	...
	const sceneModel_t	*currentModel;
	color4ub_t		entityColor;
	byte			color2D[4];
	float			sceneZfar;
	...
	qboolean		doingStencil;
	qboolean		traceShader;
...
typedef struct {
	...
	color4ub_t				identityLightColor;	// identityLight * 255
...
typedef struct {
	trRefdef_t				*refdef;
	viewParms_t				*viewParms;
	srfEntity_t				*entities;
	scenePortal_t			*portals;
	sceneModel_t			*models;
	qboolean				usedRefDef;
	struct backEndData_s	*backEnd;
	int						numDrawSurfs;
} trScene_t;
...
extern trScene_t		trScene;


// MMEFIXME: very very VERY dirty HACK! Ties renderer to Q3!
...
extern cvar_t	*r_railCoreRotate;
extern cvar_t	*r_railRingsAlign;
...
extern cvar_t	*r_alphabits;			// number of desired alpja bits
...
extern	cvar_t	*r_drawmodels;			// disable/enable model rendering
...
extern	cvar_t	*r_showtriggers;		//Show trigger brushes
...
static ID_INLINE int R_SortToIndex( unsigned sort ) {
	return ( sort & QSORT_INDEX_MASK ) >> QSORT_INDEX_SHIFT;
}
static ID_INLINE int R_SortToFog( unsigned sort ) {
	return ( sort & QSORT_FOG_MASK ) >> QSORT_FOG_SHIFT;
}
static ID_INLINE shader_t *R_SortToShader( unsigned sort ) {
	return tr.sortedShaders[ ( sort & QSORT_SHADER_MASK ) >> QSORT_SHADER_SHIFT ];
}
static ID_INLINE const surfaceType_t *R_DataToSurf( const drawSurf_t *draw ) {
	unsigned int sort = draw->sort;
	if ( sort & QSORT_HAVEMODEL_MASK ) {
		int index = R_SortToIndex( sort );
		return ((drawModel_t*)draw->data)->surface[index];
	} else {
		return ( surfaceType_t *) draw->data;
	}
}
...
void R_RotateForEntity( const sceneModel_t *ent, const viewParms_t *viewParms, orientationr_t *or );
...
*/
typedef struct {
	const color4ub_t	colors[SHADER_MAX_VERTEXES];
	const vec2_t		texCoords[2][SHADER_MAX_VERTEXES];
} stageVars_t;

typedef struct {
	const color4ub_t	*colors;
	const vec2_t		*texCoords[2];
} stageData_t;

typedef struct {
//	glIndex_t	indexes[SHADER_MAX_INDEXES];
//	vecSimd_t	xyz[SHADER_MAX_VERTEXES];
//	vecSimd_t	normal[SHADER_MAX_VERTEXES];
//	vec2_t		texCoords[SHADER_MAX_VERTEXES];
//	vec2_t		lightMap[SHADER_MAX_VERTEXES];
//	color4ub_t	colors[SHADER_MAX_VERTEXES];

	vec2_t		stageCoords[2][SHADER_MAX_VERTEXES];
	color4ub_t	stageColors[SHADER_MAX_VERTEXES];
//	int			dlightIndexes[SHADER_MAX_INDEXES];
//	vec2_t		dlightCoords[SHADER_MAX_VERTEXES];
} shaderStorage_t;
...
static ID_INLINE qboolean RB_CheckOverflow( int verts, int indexes ) {
...
void R_AddBrushModelSurfaces( const sceneModel_t *ent );
void R_AddWorldSurfaces( void );
qboolean R_CullSurface( const surfaceType_t *surface, const shader_t *shader );
...
void R_SetupEntityLighting( sceneModel_t *ent );
...
void RE_AddModelToScene( const refModel_t *ent );
void RE_AddSpriteToScene( const refSprite_t *ent );
...
void R_AddMD3Surfaces( sceneModel_t *ent );
void R_AddAnimSurfaces( sceneModel_t *ent );
void RB_SurfaceAnim( md4Surface_t *surfType );
#ifdef RAVENMD4
void R_MDRAddAnimSurfaces( const sceneModel_t *ent );
...
typedef struct {
	...
	mmeShotFormat_t format;
	float			fps;
	mmeShotEntry_t	main, stencil, depth;
} mmeShotData_t;
...
void *R_GetCommandBuffer( renderCommand_t cmd, int bytes );
void RB_ExecuteRenderCommands( const void *data );
