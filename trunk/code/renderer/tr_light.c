...
=================
*/
static void R_SetupEntityLightingGrid( sceneModel_t *ent ) {
	vec3_t	lightOrigin;
	int		pos[3];
	int		i, j;
	byte	*gridData;
...
===============
*/
static void LogLight( sceneModel_t *ent ) {
	int	max1, max2;
...
void R_SetupEntityLighting( sceneModel_t *ent ) {
	...
	ent->lightDir[0] = DotProduct( lightDir, ent->axis[0] );
	ent->lightDir[1] = DotProduct( lightDir, ent->axis[1] );
	ent->lightDir[2] = DotProduct( lightDir, ent->axis[2] );
}
...
int R_LightForPoint( vec3_t point, vec3_t ambientLight, vec3_t directedLight, vec3_t lightDir )
{
	...
	//Doesn't really need to be fully cleared, and it's basically still slow... :)
