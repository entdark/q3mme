...
//		int error = qglGetError( );
		GL_CheckErrors();
		ri.Error( ERR_DROP, "R_CreateImage: Couldn't alloc texture handler\n");
	}
	tr.numImages++;
...
static void R_CreateDlightImage( void ) {
	...
	R_LoadImage("data/dlight.tga", &pic, &width, &height);
	if (pic) {                                    
		Com_Printf("DLIGHT\n");
		tr.dlightImage = R_CreateImage("*dlight", pic, width, height, qfalse, qfalse, GL_CLAMP );
		Z_Free(pic);
...
//	}
	...
	tr.identityLightColor[0] = tr.identityLightByte;
	tr.identityLightColor[1] = tr.identityLightByte;
	tr.identityLightColor[2] = tr.identityLightByte;
	tr.identityLightColor[3] = 255;
	for ( i = 0; i < SHADER_MAX_VERTEXES; i++) 
		Byte4Copy( tr.identityLightColor, tr.colorIdentityLight[i] );
