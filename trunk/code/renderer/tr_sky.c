...
void RB_ClipSkyPolygons( void )
{
	...
	for ( i = 0; i < tess.numIndexes; i += 3 )
	{
		for (j = 0 ; j < 3 ; j++) 
		{
			VectorSubtract( xyz[indexes[i+j]],
...
static void FillCloudySkySide( const int mins[2], const int maxs[2], qboolean addIndexes )
{
	...
	for ( t = mins[1]+HALF_SKY_SUBDIVISIONS; t <= maxs[1]+HALF_SKY_SUBDIVISIONS; t++ )
	{
		for ( s = mins[0]+HALF_SKY_SUBDIVISIONS; s <= maxs[0]+HALF_SKY_SUBDIVISIONS; s++ )
		{
			VectorAdd( s_skyPoints[t][s], backEnd.viewParms.or.origin, xyz[0] );
			texCoords[0][0] = s_skyTexCoords[t][s][0];
			texCoords[0][1] = s_skyTexCoords[t][s][1];
	...
	if ( addIndexes ) {
		...
		{	
			...
			{
				indexes[0] = vertexStart + s + t * ( sWidth );
				indexes[1] = vertexStart + s + ( t + 1 ) * ( sWidth );
				indexes[2] = vertexStart + s + 1 + t * ( sWidth );
				indexes[3] = vertexStart + s + ( t + 1 ) * ( sWidth );
				indexes[4] = vertexStart + s + 1 + ( t + 1 ) * ( sWidth );
				indexes[5] = vertexStart + s + 1 + t * ( sWidth );
				tess.numIndexes += 6;
				indexes += 6;
			}
		}
...
void RB_DrawSun( void ) {
	...
	RB_BeginSurface( tr.sunShader, tess.fogNum, 0 );
