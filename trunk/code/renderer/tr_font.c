...
freetypeFont:
  ...
  //Check if we've already loaded this font/pointsize combination
	...
	while ( freeTypeFont ) {
		if (Q_stricmp( fontName, freeTypeFont->fontName ) && freeTypeFont->fontInfo.glyphScale == glyphScale ) {
	...
	while ( i <= GLYPH_END ) {
		...
		if (xOut == -1 || yOut == -1 || i == GLYPH_END)  {
			...
			Com_sprintf (name, sizeof(name), "%s_%i_%i.tga", fontName, imageNumber++, pointSize);
	...
	freeTypeFont = ri.Malloc( sizeof( *freeTypeFont ));
