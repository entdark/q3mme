...
void Console_Key (int key) {
	...
	if ( key == K_ENTER || key == K_KP_ENTER ) {
		...
		if ( (clc.demoplaying || cls.state != CA_ACTIVE) && g_consoleField.buffer[0] != '\\' 
...
void CL_KeyEvent (int key, qboolean down, unsigned time) {
  ...
  if (key == K_ENTER)
  {
	...
	if ( down && ( key < 128 || key == K_MOUSE1 ) && ( clc.demoplaying || cls.state == CA_CINEMATIC ) && !cls.keyCatchers) {
		// MME: By default now, any key doesn't stop a demo
		if (mme_anykeystopsdemo->integer != 0) {
			Cvar_Set ("nextdemo","");
			key = K_ESCAPE;
		}
	}
	...
	if ( uivm && (cls.keyCatchers & KEYCATCH_UI) ) {
		qboolean ret = VM_Call( uivm, UI_KEY_EVENT, key, down );
		if ( cls.keyCatchers & KEYCATCH_UIEXEC )  {
			if ( ret )
				return;
		} else {
			return;
	...
	if ( cgvm && (cls.keyCatchers & KEYCATCH_CGAME) ) {
		qboolean ret = VM_Call( cgvm, CG_KEY_EVENT, key, down );
		if ( cls.keyCatchers & KEYCATCH_CGAMEEXEC )  {
			if ( ret )
				return;
		} else {
			return;
