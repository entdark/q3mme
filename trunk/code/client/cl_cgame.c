...
void CL_ConfigstringModified( void ) {
	...
	for ( i = 0 ; i < MAX_CONFIGSTRINGS ; i++ ) {
		...
		if (i == 20) {
			const char *cv = Cvar_VariableString( "mme_gameoverride" );
			if (cv[0]) {
				strncpy( tempBuf, cv, sizeof( tempBuf ));
				dup = tempBuf;
			}
		}
...
// ZOID
	...
	case CG_S_ADDLOOPINGSOUND:
		S_LoopingSound( ((char *)cl.entityBaselines) + args[1], VMA(2), VMA(3), args[4], 127 );
		return 0;
	case CG_S_ADDREALLOOPINGSOUND:
		S_LoopingSound( ((char *)cl.entityBaselines) + args[1], VMA(2), VMA(3), args[4], 90 );
		return 0;
...
//		S_StopLoopingSound( args[1] );
	...
	case CG_GETCURRENTSNAPSHOTNUMBER:
		if (clc.newDemoPlayer) 
			demoGetSnapshotNumber( VMA(1), VMA(2) );
		else
			CL_GetCurrentSnapshotNumber( VMA(1), VMA(2) );
		return 0;
	case CG_GETSNAPSHOT:
		if (clc.newDemoPlayer) 
			return demoGetSnapshot( args[1], VMA(2) );
		else
...
*/
	...
	case CG_R_ADDSPRITETOSCENE:
		...
		return 0;
	case CG_R_ADDMODELTOSCENE:
		re.AddModelToScene( VMA(1) );
		return 0;
	case CG_R_DECALRESET:
		re.DecalReset();
		return 0;
	case CG_R_DECALPROJECT:
		re.DecalProject( args[1], VMA(2), VMA(3), VMA(4), VMF(5), VMF(6), args[7], args[8]);
		return 0;
...
void CL_InitCGame( void ) {
	...
	//CaNaBiS, force a native binary first
