...
extern qboolean GLW_CreateWindow( const char *drivername, int width, int height, int colorbits, qboolean cdsFullscreen, qboolean hide );
//extern qboolean GLW_CreateWindow( const char *drivername, int width, int height, int colorbits, qboolean cdsFullscreen );
										   ...
										   qboolean cdsFullscreen )
{
	...
	if (!qwglGetExtensionsStringARB && ( r_multiSample->integer || ( mme_renderWidth->integer && mme_renderHeight->integer )) ) {
		if (GLW_CreateWindow( drivername, 320, 200, 0, qfalse, qtrue )) {
...
static int GLW_ChoosePFD( HDC hDC, PIXELFORMATDESCRIPTOR *pPFD )
{
	...
	ri.Printf( PRINT_ALL, "...GLW_ChoosePFD( %d, %d, %d %d )\n", ( int ) pPFD->cColorBits, ( int ) pPFD->cDepthBits, ( int ) pPFD->cStencilBits, ( int ) pPFD->cAlphaBits );
	...
	for ( i = 1; i <= maxPFD; i++ )
	{
		...
		if ((pfds[i].cAlphaBits < 8) && (pPFD->cAlphaBits > 0)) {
			continue;
		}
...
*/
static void GLW_CreatePFD( PIXELFORMATDESCRIPTOR *pPFD, int colorbits, int depthbits, int stencilbits, qboolean stereo, int alphabits )
{
...
static int GLW_MakeContext( PIXELFORMATDESCRIPTOR *pPFD )
{
	...
	if ( !glw_state.hGLRC )
	{
		...
		if ( ( glw_state.hGLRC = qwglCreateContext( glw_state.hDC ) ) == 0 )
		{
			...
			if (r_multiSample->integer) {
				ri.Printf( PRINT_ALL, "...Trying again without multisampling\n" );
				ri.Cvar_Set( "r_multiSample", "0" );
				return TRY_PFD_FAIL_SOFT;
			}
...
{
	int		tpfd;
	int		depthbits, stencilbits, alphabits;
	...
	{
		GLW_CreatePFD( &pfd, colorbits, depthbits, stencilbits, r_stereo->integer, alphabits );
		if ( ( tpfd = GLW_MakeContext( &pfd ) ) != TRY_PFD_SUCCESS )
		{
			...
			GLW_CreatePFD( &pfd, colorbits, depthbits, 0, r_stereo->integer, alphabits );
...
*/
qboolean GLW_CreateWindow( const char *drivername, int width, int height, int colorbits, qboolean cdsFullscreen, qboolean hide )
{
	RECT			r;
	...
	if ( g_wv.hWnd && qwglChoosePixelFormatARB ) {
		if ( glw_state.hDC ) {
			if (glw_state.hGLRC) {
				qwglDeleteContext( glw_state.hGLRC );
				glw_state.hGLRC = NULL;
	...
	{
		MoveWindow( g_wv.hWnd, x, y, w, h, 1 );
							...
							qboolean cdsFullscreen )
{
	...
	// do a CDS if needed
	if ( !winHideWindow && cdsFullscreen )
	{
		...
		if ( glw_state.cdsFullscreen )
		{
			...
			if ( !GLW_CreateWindow ( drivername, glConfig.vidWidth, glConfig.vidHeight, colorbits, qtrue, qfalse ) )
			{
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					if ( !GLW_CreateWindow( drivername, glConfig.vidWidth, glConfig.vidHeight, colorbits, qfalse, qfalse ) )
					{
						return RSERR_INVALID_MODE;
					}
	...
	else
	{
		...
		glw_state.cdsFullscreen = qfalse;
		if ( !GLW_CreateWindow( drivername, glConfig.vidWidth, glConfig.vidHeight, colorbits, qfalse, winHideWindow ) )
		{
			return RSERR_INVALID_MODE;
		}
	}
...
static void GLW_InitExtensions( void )
{
	...
	glMMEConfig.framebufferObject = 0;
	glMMEConfig.shaderSupport = 0;
	...
	glMMEConfig.framebufferMultiSample = 0;
	if ( strstr( glConfig.extensions_string, "GL_EXT_framebuffer_multisample") && strstr( glConfig.extensions_string, "GL_EXT_framebuffer_blit")) {
		...
		glMMEConfig.framebufferMultiSample = 1;
	}
...
static qboolean GLW_LoadOpenGL( const char *drivername )
{
	...
	if ( QGL_Init( buffer ) ) 
	{
		...
		for( pass = 10; pass > 0; pass-- ) {
			...
			if ( GLW_StartDriverAndSetMode( drivername, r_mode->integer, r_colorbits->integer, r_fullscreen->integer ) )
				break;
...
void GLimp_Minimize( void ) {
	if ( glw_state.isSuspended )
		return;
	if ( glw_state.cdsFullscreen ) {
		memset( &glw_state.suspendMode, 0, sizeof( glw_state.suspendMode ) );
		glw_state.suspendMode.dmSize = sizeof( glw_state.suspendMode );
		if ( !EnumDisplaySettings( NULL, ENUM_CURRENT_SETTINGS, &glw_state.suspendMode ))
			return;
		ChangeDisplaySettings( NULL, 0 );
	}
	//Set to true now to prevent recursion
	glw_state.isSuspended = qtrue;
...
void GLimp_Restore( void ) {
	if ( !glw_state.isSuspended )
		return;
	glw_state.isSuspended = qfalse;
	if ( glw_state.cdsFullscreen ) {
		ChangeDisplaySettings( &glw_state.suspendMode, CDS_FULLSCREEN );
