...
#define dp_realtime uis.realtime
...
static void UI_LegsSequencing( playerInfo_t *pi ) {
	...
	if ( pi->legsAnimationTimer > 0 ) {
		if ( currentAnim == LEGS_JUMP ) {
			float time = pi->legsAnimationTimer - ( UI_TIMER_JUMP / 2);
			float val = (time / ( UI_TIMER_JUMP / 2)) * sqrt( JUMP_HEIGHT );
			pi->jumpHeight = JUMP_HEIGHT - val * val;
		}
		return;
	}
...
===============
*/
static void UI_PlayerFloatSprite( playerInfo_t *pi, const vec3_t origin, qhandle_t shader ) {
	refEntity_t		ent;
...
===============
*/
void UI_RenderPlayer( const vec3_t baseOrigin, playerInfo_t *pi ) {
	int				renderfx;
	refEntity_t		legs;
	refEntity_t		torso;
	refEntity_t		head;
	...
	// add muzzle flash
	//
	if ( dp_realtime <= pi->muzzleFlashTime || ( pi->weapon == WP_GAUNTLET && pi->torsoAnim == TORSO_ATTACK2 )) {
		if ( pi->flashModel ) {
...
qboolean UI_RegisterClientModelname( playerInfo_t *pi, const char *modelSkinName ) {
	...
	Q_strncpyz( pi->modelName, modelName, sizeof( pi->modelName ));
	return qtrue;
}
...
===============
*/
void UI_PlayerInfo_Sound( playerInfo_t *pi, const char *sound ) {
	char soundName[MAX_QPATH];
	sfxHandle_t handle;

	Com_sprintf( soundName, sizeof( soundName ), "sound/player/%s/%s.wav", pi->modelName, sound );
	handle = trap_S_RegisterSound( soundName, qfalse );
	if (!handle)
		return;
	trap_S_StartLocalSound( handle, 0 );
}
