...
sfxHandle_t ScrollList_Key( menulist_s *l, int key )
{
	...
	switch (key)
	{
		case K_ENTER:
		case K_KP_ENTER:
			if ((l->generic.flags & QMF_HASMOUSEFOCUS) && l->generic.callback && (l->generic.flags & QMF_DOUBLECLICK)) {
		...
		case K_MOUSE1:
			if (l->generic.flags & QMF_HASMOUSEFOCUS)
			{
				...
				deltatime = uis.realtime - l->lastclick;
				l->lastclick = uis.realtime;
				...
				if ( l->generic.callback && (deltatime < 300) && (l->generic.flags & QMF_DOUBLECLICK)) {
					l->generic.callback( l, QM_ACTIVATED );
					return (menu_move_sound);
				}
