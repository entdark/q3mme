...
int Q_PrintStrlen( const char *string ) {
	...
	p = string;
	while( *p ) {
		int colorLen = Q_parseColorString( p, 0);
		if( colorLen ) {
			p += colorLen;
			continue;
		}
...
char *Q_CleanStr( char *string ) {
	...
	d = string;
	while ((c = *s) != 0 ) {
		int colorLen = Q_parseColorString( s, 0);
		if ( colorLen ) {
			s += colorLen;
			continue;
		}		
...
MME: fixes a bug when infostring doesn't begin with backslash
==================
*/
void Info_SetValueForKey( char *s, const char *key, const char *value ) {
	...
	if (s[0] != '\\') needbackslash = 1;

	if (strlen(newi) + strlen(s) + needbackslash > MAX_INFO_STRING)
	{
		Com_Printf ("Info string length exceeded\n");
		return;
	}

	if (needbackslash) strcat (newi, "\\");
	strcat (newi, s);
	strcpy (s, newi);
}
