...
void ByteToDir( int b, vec3_t dir ) {
	if ( b < 0 || b >= NUMVERTEXNORMALS ) {
		...
		dir[0] = 1;
//		Com_Printf("WTF illegal byte dir\n");
		return;
	}
...
This is not implemented very well..., but it is now!
===============
*/
...
void VectorRotateDirect( vec3_t v, const vec3_t matrix[3] ) {
	...
	v[0] = DotProduct( src, matrix[0] );
	v[1] = DotProduct( src, matrix[1] );
	v[2] = DotProduct( src, matrix[2] );
