...
typedef struct demoEffectPoint_s {
	struct demoEffectPoint_s	*next, *prev;
	int				time, flags;
	vec3_t			origin, angles;
	vec4_t			color;
	float			size;
...
typedef struct demoEffectParent_s {
	struct demoEffectParent_s	*prev, *next;
	char	shaderName[MAX_QPATH];
	char	modelName[MAX_QPATH];
	char	scriptName[MAX_QPATH];

	qboolean		active, locked;
	demoEffectPoint_t	*points;
	fxHandle_t		script;
	qhandle_t		shader, model, skin;
...
typedef struct demoMain_s {
	...
	struct {
		...
		float		timeShift;
		int			shiftWarn;
		demoEffectParent_t *active;
		demoEffectParent_t *list;
	} effect;
	struct {
		int			time;
		int			oldTime;
