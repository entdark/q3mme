...
static void CG_CPMAParseSocres( int i ) {
	areaInfo_t* area = cgs.areaInfo + i;
	const char *s = CG_ConfigString( 710 + i );
	area->redScore = atoi( Info_ValueForKey( s, "sr" ) );
	area->blueScore = atoi( Info_ValueForKey( s, "sb" ) );
}
/*
================
...
static void CG_ConfigStringModified( void ) {
	...
	if ( mov_debug.integer & 2 ) 
		 CG_Printf( "CS Modifed %d to %s\n", num, str );
	...
	/* Handle the scores with mode specific ones */
	if ( cg.cpma.detected ) {
	...
	while (*str) {
		if (len > TEAMCHAT_WIDTH - 1) {
			...
			*p = 0;
			...
			cgs.teamChatPos++;
			p = cgs.teamChatMsgs[cgs.teamChatPos % chatHeight];
			memcpy( p, color, colorLen );
			p += colorLen;
			len = 0;
			ls = NULL;
		}

		newColorLen = Q_parseColorString( str , 0);
		if (newColorLen) {
			colorLen = newColorLen;
			if (newColorLen > sizeof( color )) {
				CG_Printf("Extremely long color code? skipping\n");
				return;
			}
			memcpy( color, str, colorLen );
			memcpy( p, str, colorLen );
			str += colorLen;
			p += colorLen;
			continue;
...
static void CG_ServerCommand( void ) {
  ...
  if ( Q_stricmp (cmd, "remapShader") == 0 ) {
	...
	if ( !strcmp(cmd, "play" )) {
		const char *wavName = va( "%s.wav", CG_Argv(1));
		trap_S_StartLocalSound( trap_S_RegisterSound( wavName, qfalse) , CHAN_LOCAL_SOUND );
		return;
	}
