...
typedef struct {
	...
	vec3_t			color1;
	vec3_t			color2;
	byte			c1RGBA[4], c2RGBA[4];
	byte			legsRGBA[4], torsoRGBA[4], headRGBA[4];
	...
	int				armor;
	int				curWeapon;
	unsigned int	area;			//What area are we in
...
typedef struct {
	cgWeaponFX_t grapple, rocket, grenade, plasma, bfg, rail, lightning, gauntlet, shotgun, machinegun;
	fxHandle_t	bubbles;
	struct {
		fxHandle_t gibbed, haste, quad;
		fxHandle_t teleportIn, teleportOut;
		fxHandle_t talk, connection, impressive, excellent, gauntlet, capture, assist, defend;
		fxHandle_t holyShit, friend;
	} player;
} cgFX_t;
...
//New MME cvars
extern	vmCvar_t		cg_drawDamageBlob;
...
extern	vmCvar_t		cg_centerScale; 
...
void CG_Text_Paint(float x, float y, float scale, vec4_t color, const char *text, qboolean shadowed );
...
void CG_BubbleTrail( const vec3_t start, const vec3_t end, void *parent );
void CG_SpawnEffect( vec3_t org );
void CG_ScorePlum( int client, vec3_t org, int score );

void CG_GibPlayer( const vec3_t playerOrigin, const vec3_t playerDirection );
...
//New mme traps for direct entity types
void trap_R_AddSpriteToScene( const refSprite_t *sprite );
void trap_R_AddModelToScene( const refModel_t *sprite );
void trap_R_DecalProject( qhandle_t shader, const vec3_t origin, const vec3_t dir, const vec4_t color, float orientation, float radius, int flags, int lifeTime );
void trap_R_DecalReset( void );
