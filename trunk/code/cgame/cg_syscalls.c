...
int trap_MME_DemoInfo( mmeDemoInfo_t *info ) {
	return syscall( CG_MME_DEMOINFO, info );
}
...
void trap_R_AddModelToScene( const refModel_t *model ) {
	syscall( CG_R_ADDMODELTOSCENE, model );
}
...
void trap_R_DecalProject( qhandle_t shader, const vec3_t origin, const vec3_t dir, const vec4_t color, float orientation, float radius, int flags, int lifeTime ) {
	syscall( CG_R_DECALPROJECT, shader, origin, dir, color, PASSFLOAT(orientation), PASSFLOAT( radius ), flags, lifeTime );
}

void trap_R_DecalReset( void ) {
	syscall( CG_R_DECALRESET );
}
...
void trap_FX_Begin( int time, float fraction ) {
	syscall( CG_FX_BEGIN, time, PASSFLOAT( fraction ) );
}
void trap_FX_End( void ) {
	syscall( CG_FX_END );
}
fxHandle_t trap_FX_Register( const char *name ) {
	return syscall( CG_FX_REGISTER, name );
}
