...
void QuatTimeSpline2( float t, int t0, int t1, int t2, int t3, const Quat_t q0, const Quat_t q1, const Quat_t q2, const Quat_t q3, Quat_t qr) {  
	Quat_t	qn, qt;
	Quat_t	qlog10, qlog21, qlog32;
	Quat_t	qa, qb;
	float scale;
	...
	/* Create some logs */
	QuatConjugate( q0, qn);
	QuatMultiply( qn, q1, qt );
	QuatLog( qt, qlog10 );

	QuatConjugate( q1, qn);
	QuatMultiply( qn, q2, qt );
	QuatLog( qt, qlog21 );

	QuatConjugate( q2, qn);
	QuatMultiply( qn, q3, qt );
	QuatLog( qt, qlog32 );
	
	/* Create Outgoing */
	scale = (float)( t2 - t1) / (float)( t2 - t0);
	QuatAdd( qlog21, qlog10, qt );
	QuatScale( qt, scale, qt );
	QuatSub( qt, qlog21, qt );
	QuatScale( qt, 0.5f, qt );
	QuatExp( qt, qt );
	QuatMultiply( q1, qt, qb );

	/* Create incoming */
	scale = (float)( t2 - t1) / (float)( t3 - t1);
	QuatAdd( qlog32, qlog21, qt );
	QuatScale( qt, scale, qt );
	QuatSub( qlog21, qt, qt );
	QuatScale( qt, 0.5f, qt );
	QuatExp( qt, qt );
	QuatMultiply( q2, qt, qa );

	/* Slerp the final result */
	QuatSlerp( t, q1, q2, qt );
	QuatSlerp( t, qa, qb, qn );
	QuatSlerp( 2*t*(1 - t), qt, qn, qr );
