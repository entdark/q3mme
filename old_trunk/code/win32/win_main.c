...
void Sys_Init( void ) {
	...
	// MME: add variables
	mme_roundmode = Cvar_Get ("mme_roundmode", "1", 0);
...
//    _controlfp( -1, _MCW_EM  ); // no exceptions, even if some crappy
		...
		// run the game
		Com_Frame();
		if (mme_roundmode->modified) {
			if (mme_roundmode->integer) {
				Sys_SnapVectorCurrent = Sys_SnapVectorCast;
				Com_Printf("Rounding mode set to cast (quality)\n");
			} else {
				Sys_SnapVectorCurrent = Sys_SnapVectorFTOL;
				Com_Printf("Rounding mode set to fast (original q3)\n");
			}
			mme_roundmode->modified = qfalse;
		}
