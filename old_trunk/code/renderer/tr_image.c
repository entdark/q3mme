...
void SaveJPG(char * filename, int quality, int image_width, int image_height, unsigned char *image_buffer) {
  ...
  if (!mme_jpeg_downsample_chroma->integer) {
	  int i;
  ...
  if (mme_jpeg_optimize_huffman_tables->integer) cinfo.optimize_coding = 1;
...
static int SaveTGA_RLEPacket(byte *out, byte* pixel, int pixelsize, int count) {
	byte rlepacketheader;
	int i;

	if (count < 1) {
		Com_DPrintf(S_COLOR_YELLOW "WARNING: SaveTGA_RLEWrite() count is zero\n");
		return 0;	// Nothing to write,
	}
	if (count > 128) {
		Com_DPrintf(S_COLOR_YELLOW "WARNING: SaveTGA_RLEWrite() count is > 128\n");
		return 0;
	}
	if ((out == NULL) || (pixel == NULL)) {
		Com_DPrintf(S_COLOR_YELLOW "WARNING: SaveTGA_RLEWrite() has been passed NULL\n");
		return 0;	// Nothing to write,
	}
...
static int SaveTGA_RLEPacketRaw(byte *out, byte* data, int pixelsize, int count) {
	byte rlepacketheader;
	int i;

	if (count < 1) {
		Com_DPrintf(S_COLOR_YELLOW "WARNING: SaveTGA_RLEWriteRaw() count is zero\n");
		return 0;	// Nothing to write,
	}
	if (count > 128) {
		Com_DPrintf(S_COLOR_YELLOW "WARNING: SaveTGA_RLEWriteRaw() count is > 128\n");
		return 0;
	}
	if (out == NULL) {
		Com_DPrintf(S_COLOR_YELLOW "WARNING: SaveTGA_RLEWriteRaw() has been passed NULL\n");
		return 0;	// Nothing to write,
	}
...
#define PIXEL_READ(pixel) Com_Memcpy(pixel, inbuffer, pixelsize);inbuffer+=4;read_pos++;
#define PIXEL_COPY(pixel1,pixel2) Com_Memcpy(pixel1, pixel2, pixelsize);
...
static int SaveTGA_RLEEncode(byte *out, const int image_width, const int image_height, const void* image_buffer, const int image_buffer_size, const int image_hasalpha) {
	int pixelsize = (image_hasalpha) ? 4 : 3;
	byte *inbuffer = (byte*)image_buffer;
...
===============
*/
void SaveTGA(const char * filename, const int image_compressed, const int image_width, const int image_height, byte *image_buffer, const int image_buffer_size, const int image_hasalpha) {
	int i, temp;
	byte *out;
	int outbuffersize = image_buffer_size * 1.1; // Just in case RLE results in larger file

	int filesize = 18;	// header is here by default

	out = malloc(outbuffersize); 
	if (!out) {
		Com_Printf (S_COLOR_YELLOW "WARNING: Not enough memory for TGA saving");		// yellow
		return;
	}
	Com_Memset(out, 0, outbuffersize);
	...
	if (!image_compressed) { // Plain memcpy
		if (image_hasalpha) {
			Com_Memcpy(out+filesize, image_buffer, image_buffer_size);
			filesize += image_buffer_size;
		} else {
			byte *buftemp = out+filesize;
			for (i = 0; i < image_buffer_size; i += 4) {
	...
	ri.FS_WriteFile(filename, out, filesize);
	free(out);
	return;
}
